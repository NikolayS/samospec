...
//
// `subscription_auth: true` means the adapter is running via OAuth
// (browser login). Work calls proceed normally; token accounting is
// unavailable — samospec uses wall-clock + iteration caps instead
// (SPEC §11 subscription-auth escape).
//
// `usable_for_noninteractive` is kept in the schema for back-compat
// with existing test fixtures but is no longer enforced by adapters.
// Work calls always attempt to spawn regardless of this field.
export const AuthStatusSchema = z.object({
  authenticated: z.boolean(),
  account: z.string().optional(),
  expires_at: z.string().optional(),
  subscription_auth: z.boolean().optional(),
  // Informational only; no longer used for enforcement (see #48).
