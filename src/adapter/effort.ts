...
//   3. The unified default {@link UNIFIED_DEFAULT_EFFORT} — `"high"`
//      (deep, strong review by default), NOT `"max"`.
//
// Before this module every lead call defaulted to `"max"` (scattered
// `input.effort ?? "max"` fallbacks + hardcoded `effort: "max"` in the
// round runner). That gave the user no single knob to dial depth vs
// speed for all seats at once. This module centralises the resolution so
// the default is consistent (`high` — deep by default, with `--effort
// max` available for the deepest review) and the `--effort` flag is
// honoured everywhere.
 ...
 * `--effort` flag nor a per-seat config value pins it. `"high"` — a deep,
 * strong default so the out-of-the-box experience is review-grade.
 * Deliberately NOT `"max"`: `--effort max` is the explicit knob for the
 * deepest (and slowest) review.
 */
export const UNIFIED_DEFAULT_EFFORT: EffortLevel = "high";
 ...
 * precedence: flag > per-seat config > unified high default.
 */
 ...
 * and finally to the unified high default.
 */
...
export const EFFORT_PROMPT_INTRO =
  ...
  "  high   — deep (default);   ~15-30 min/round\n" +
  "  medium — balanced;         ~5-12 min/round\n" +
...
export const EFFORT_PROMPT_CHOOSE =
  "Choose [max/high/medium/low/off] (default high): ";

/**
 * Resolve the answer to the interactive effort prompt. Empty input
 * (just Enter) selects the unified high default. An unrecognised
